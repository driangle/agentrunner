...
// invoke the Gemini CLI programmatically, covering basic usage, streaming,
...
// Run:
//   npx tsx main.ts
//   npx tsx main.ts --binary /path/to/gemini
...
import {
  ...
  messageToolName,
  messageToolOutput,
} from "agentrunner/gemini";

const { values } = parseArgs({
  options: {
    binary: { type: "string", default: "gemini" },
...
async function exampleSimpleRun(runner: ReturnType<typeof createGeminiRunner>) {
...
async function exampleStreaming(runner: ReturnType<typeof createGeminiRunner>) {
...
async function exampleSession(runner: ReturnType<typeof createGeminiRunner>) {
  const prompt =
